...
class Pyyctest:
    ...
    def base_of_testname(test_py):
        ...
        return base if (ext == '.py' and not (".ipynb_checkpoints" in base) ) else None
    ...
    def run_python(self, flat=False):
        ...
        if not flat:
            ...
            if not os.path.exists(self.flatpysource):
                return Result.success
            return self.run([python_exe, self.flatpysource], self.flatoutput)

    def diff_with_python(self, flat=False):
        # type: () -> Result
        compare = self.output
        if flat:
            if not os.path.exists(self.flatoutput):
                return Result.success
            compare = self.flatoutput
        popen = subprocess.Popen(['diff', '-w', '-B', '-u', self.expected, compare],
                                 stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                 preexec_fn = lambda: os.setpgid(0, 0))
        return popen_result(popen)
...
def test_compiler(filename_py):
    ...
    try: 
        if pyyctest.run_python(flat=True) == Result.failure:
            ...
            assert pyyctest.diff_with_python(flat=True) != Result.failure
    except ValueError as err:
        pytest.skip(err.message)
